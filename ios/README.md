...
1. Replace `com.example.IAPNotifications` with your unique bundle identifier and select your Apple Developer team under **Signing & Capabilities**. Keep **Push Notifications** and **Sign in with Apple** enabled. Register the same bundle ID in Firebase project `the-app-quest` and enable the **Apple** authentication provider only. Enable Sign in with Apple on that App ID in Apple Developer. The native-only flow needs no web Services ID or Firebase client SDK/plist. The server exchanges Apple credentials with Firebase and returns an opaque service session. Real-device testing needs an Apple Account with two-factor authentication signed into iCloud.
